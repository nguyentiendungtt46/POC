...
<tiles:insertDefinition name="catalog">
	<tiles:putAttribute name="title" value="Giám sát service" />
					...
					<div class="Table" id="divSearchInf">
						<div class="row">
							<div class="row title-page" style="adding-bottom: 20px;">
								<h1>Giám sát service</h1>
							</div>
							<div class="label-static col-md-2 col-lg-2 col-sm-12 col-xs-12">Thời
								gian nhận từ ngày</div>
							<div class="col-md-3 col-lg-3 col-sm-12 col-xs-12">
								<form:input path="fromDate" id="fromDate" cssClass="start_date"
									value="${startDate}"></form:input>
							</div>
							<div class="label-static col-md-2 col-lg-2 col-sm-12 col-xs-12">Đến
								ngày</div>
							<div class="col-md-3 col-lg-3 col-sm-12 col-xs-12">
								<form:input path="toDate" id="toDate" cssClass="start_date"
									value="${endDate}"></form:input>
							</div>
						</div>
						...
						<div class="row">
							...
							<div class="col-md-3 col-lg-3 col-sm-12 col-xs-12">
								<form:select class="form-control" path="serviceId"
									onchange="reloadProduct($(this).val())" title="Chọn service">
							...
							<div class="label-static col-md-2 col-lg-2 col-sm-12 col-xs-12">Sản
								phẩm</div>
							<div class="col-md-3 col-lg-3 col-sm-12 col-xs-12">
								<form:select path="prodId" id="prodId" class="form-control"
									title="Chọn service">

									<option value="">- Chọn -</option>
									<c:forEach items="#{catProducts}" var="item">
										<option value="${item.id}">
											<c:out value="${item.code}-${item.name}" />
										</option>
									</c:forEach>
								</form:select>
							</div>
						...
						<div class="row">
							...
							<div class="col-md-3 col-lg-3 col-sm-12 col-xs-12">
								<form:select path="type">
									<option value="">- Chọn -</option>
									<option value="1">1-Đi</option>
									<option value="2">2-Đến</option>
								</form:select>
							</div>
						...
						<div class="divaction" align="center">
							<input class="btn blue" type="button" onclick="findData();"
								...
								onclick="exportExcel();" value="Export" />
						</div>
					</div>
				</div>
	...
	<tiles:putAttribute name="extra-scripts">
		<script type="text/javascript">
			function reloadProduct() {
				...
				$.ajax({
					url : "logServices?method=reloadProduct",
					success : function(res) {
						...
						$('#prodId').empty();
						$('#prodId').append('<option value="">Chọn</option>');
						$('#prodId').select2({
							data : data
						});
					},
			...
			function inputDetail(id) {
				$.ajax({
					url : $('#theForm').attr('action')
							+ '?method=loadInAndOut&id=' + id + '&type=in',
					method : 'GET',
			...
			function outputDetail(id) {
				$.ajax({
					url : $('#theForm').attr('action')
							+ '?method=loadInAndOut&id=' + id + '&type=out',
					method : 'GET',
					success : function(data) {
						$(".question").empty();
						$(".question").append(
			...
			function outputPrintTrace(id) {
				$.ajax({
					url : $('#theForm').attr('action')
							+ '?method=loadInAndOut&id=' + id + '&type=error',
					method : 'GET',
					success : function(data) {
						$(".question").empty();
						$(".question").append(
			...
			function exportExcel() {
				...
				window.open($('#theForm').attr('action')
						+ '?method=exportExcel&serviceId=' + serviceId
						+ '&fromDate=' + fromDate + '&toDate=' + toDate + '&partnerId='+partnerId);
			}
		</script>
	</tiles:putAttribute>
