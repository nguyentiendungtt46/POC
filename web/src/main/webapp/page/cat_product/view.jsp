...
<tiles:insertDefinition name="catalog">
	<tiles:putAttribute name="title" value="Quản lý loại sản phẩm" />
	<tiles:putAttribute name="formInf">
		<spring:url value="/catProduct" var="formAction" />
	...
	<form:form cssClass="form-horizontal" id="theForm"
				...
				<div class="row search-style">
					<div class="Table" id="divSearchInf">
						...
						<div class="row">
		                    ...
		                    <div class="col-md-3 col-lg-3 col-sm-12 col-xs-12">
			                	<form:select class="form-control" path="server_name" title="Chọn service">
		...
		<tiles:putAttribute name="catDetail" cascade="true">
			...
			<form:hidden path="catProduct.order" id="order" />
			...
			<div class="box-custom">
				...
				<div class="row">
					...
					<div class="col-md-3 col-lg-3 col-sm-12 col-xs-12">
						<form:checkbox path="catProduct.status" />
					</div>
					<div class="label-static col-md-2 col-lg-2 col-sm-12 col-xs-12">
						Mã SP HTHTL</font>
					</div>
				...
				<div class="row">
					...
					<div class="col-md-3 col-lg-3 col-sm-12 col-xs-12">
						<form:select path="catProduct.typeProduct">
				...
				<div class="row">
					...
					<div class="col-md-3 col-lg-3 col-sm-12 col-xs-12">
						<form:checkbox path="catProduct.callByJob"></form:checkbox>
					</div>
	...
	<tiles:putAttribute name="extra-scripts">
		<script type="text/javascript">
			...
			function loadTCTDCbox() {
				$.ajax({
					url : $('#theForm').attr('action') + '?method=getListTCTD',
					data : {},
					method : 'GET',
					success : function(_result, status, xhr) {
						...
						$.each(_result, function(i, item) {
							$('.listTCTD').append(
									$('<option>', {
										value : item.id,
										text : item.code + " - " +(item.sortName == '' ? item.name
												: (item.name + '('
														+ item.sortName + ')'))
									}));
						});
								...
								success : function(_result) {
									if (_result != null) {
										...
										$('#_tree_catParIdx').tree('append', {
										 ...
										 $('#_tree_catParIdx').tree('expandTo', node.target);
									}
								}
							});
						},
						onCheck : function(node) {
							var arrChecked = $('#_tree_catParIdx').tree('getChecked');
			...
			function parentNotAvail(treeNode, arrChecked){
				...
				var parentNode = $('#_tree_catParIdx').tree(
				'getParent', treeNode.target);
				
				// Cha không co phi
				if(parentNode.price == ''
					&& parentNode.price == null
			...
			function addRowCustomer() {
				if($('#listTCTD').val()==undefined || $('#listTCTD').val().length==0){
			...
			function getCheckedNode(_strArrId){
				...
				for (var i = 0; i < _arrId.length; i++) {
					if(_arrId[i] != "" && i != _arrId.length - 1){
						var _findNode = $('#_tree_catParIdx').tree('find', _arrId[i]);
			...
			function exportExcel() {
				...
				window.open($('#theForm').attr('action') + '?method=exportExcel&code=' + code + '&name=' + name + '&serverId=' + serverId);
			}
		</script>
	</tiles:putAttribute>
