...
public class DataTableColumnSpecs {
	 ...
	 * @param i the i
	 */
	public DataTableColumnSpecs(JsonObject requestPost, int i) {
		this.setIndex(i);
		prepareColumnSpecs(requestPost, i);
	}
	 ...
	 * @param i the i
	 */
	private void prepareColumnSpecs(JsonObject requestPost, int i) {
		JsonObject column = requestPost.get("columns").getAsJsonArray().get(i).getAsJsonObject();
		JsonArray orders = requestPost.get("order").getAsJsonArray();
		this.setData(column.get("data").getAsString());
//		this.setName(request.getParameter("columns["+ i +"][name]"));
		this.setName(column.get("name").getAsString());
//		this.setOrderable(Boolean.valueOf(request.getParameter("columns["+ i +"][orderable]")));
		this.setOrderable(column.get("orderable").getAsBoolean());
//		this.setRegex(Boolean.valueOf(request.getParameter("columns["+ i +"][search][regex]")));
		this.setRegex(column.get("search").getAsJsonObject().get("regex").getAsBoolean());
//		this.setSearch(request.getParameter("columns["+ i +"][search][value]"));
		this.setSearch(column.get("search").getAsJsonObject().get("value").getAsString());
//		this.setSearchable(Boolean.valueOf(request.getParameter("columns["+ i +"][searchable]")));
		this.setSearchable(column.get("searchable").getAsBoolean());
		
//		int sortableCol = Integer.parseInt(request.getParameter("order[0][column]"));
		int sortableCol = orders.get(0).getAsJsonObject().get("column").getAsInt();
		
//		String sortDir = request.getParameter("order[0][dir]");
		String sortDir = orders.get(0).getAsJsonObject().get("dir").getAsString();
